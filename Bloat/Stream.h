...
public:
	...
	inline StreamType& GetStream() noexcept { return stream; }
	...
	inline std::vector<unsigned char> ReadBytes(const std::streamsize numBytes)
	{
	...
	inline std::string ReadString(const std::streamsize numBytes)
	{
		auto bytes = ReadBytes(numBytes);
		return std::string(bytes.begin(), bytes.end());
	}

	inline std::u8string ReadUtf8String(const std::streamsize numBytes)
	{
		auto bytes = ReadBytes(numBytes);
